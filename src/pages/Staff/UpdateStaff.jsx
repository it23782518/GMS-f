...
const UpdateStaff = () => {
  ...
  useEffect(() => {
    const fetchStaffDetails = async () => {
      try {
        setLoading(true)
        const response = await getStaffById(nic)
  ...
  const handleConfirmUpdate = async () => {
    ...
    try {
      ...
      // Email is included in dataToSubmit, so it will be sent to the API

      console.log("Submitting data:", dataToSubmit)

      const response = await updateStaff(nic, dataToSubmit)
      console.log("Update response:", response)

      setSuccess(true)
      setTimeout(() => navigate("/staff-list"), 2000)
    } catch (error) {
          ...
          <div className="bg-gradient-to-r from-rose-700 to-rose-500 p-5 sm:p-6 md:p-8 relative">
            ...
            <div className="relative">
              <div className="flex items-center mb-2 sm:mb-3">
                ...
                <h1 className="text-xl sm:text-2xl font-bold text-white drop-shadow-sm">Update Staff Member</h1>
              </div>
